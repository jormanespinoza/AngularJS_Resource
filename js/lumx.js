	...
	.service('LxDialogService', ['$timeout', '$interval', '$window', function ($timeout, $interval, $window)
		{
			...
			function checkDialogHeight(dialogId) {
				var dialogMargin = 60,
					...
					dialogScrollable = angular.element('<div/>', {
				...
				if (HeightToCheck >= $window.innerHeight) {
					...
					if (dialog.find('.dialog__scrollable').length === 0) {
						dialogScrollable.css({
							...
							bottom: dialogActions.outerHeight()
						});
	...
	.service('LxProgressService', ['$timeout', '$interval', function ($timeout, $interval)
		{
			...
			function init() {
				...
				// Linear
				progressLinear = angular.element('<div/>', {
					'class': 'progress-linear'
				});
				progressLinearBackground = angular.element('<div/>', {
					'class': 'progress-linear__background'
				});
				progressLinearFirstBar = angular.element('<div/>', {
					'class': 'progress-linear__bar progress-linear__bar--first'
				});
				progressLinearSecondBar = angular.element('<div/>', {
			...
			function showLinearProgress(color, container) {
				...
				progressLinearBackground.css({
					backgroundColor: color
				});
				progressLinearFirstBar.css({
					backgroundColor: color
				});
				progressLinearSecondBar.css({
					backgroundColor: color
				});
        ...
        function ($scope, $window, LxScrollbarService)
		{
			...
			function initScrollbar() {
				...
				if (scrollbarContentHeight <= scrollbarContainerHeight) {
					...
					scrollbarYAxisHandle.css({
						height: (scrollbarContainerHeight / scrollbarContentHeight) * 100 + '%'
					});
				}
			}
					...
					function updateTextareaHeight() {
						$timeout(function () {
							$field
								.removeAttr('style')
								.css({
									height: $field[0].scrollHeight + 'px'
								});
						});
					}
